...
class ChangeAccountTypeForm(Form):
    role = QuerySelectField('New Account type',validators=[InputRequired()],get_label='name',query_factory=
                            lambda :db.session.query(Role).order_by('permissions'))
...
class InviteUserForm(Form):
    role = QuerySelectField(
        ...
        get_label='name',
        query_factory=lambda :db.session.query(Role).order_by('permission')
    )
